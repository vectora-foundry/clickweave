...
//! Provides two paths:
//!
//! 1. [`actions_to_sketch`] — direct conversion from `WalkthroughAction` →
//!    `Vec<ActionSketchStep>` without a `Workflow` intermediary. Used by the
//!    updated `save_walkthrough_as_skill` Tauri command (Phase 1.I).
//!
//! 2. [`walkthrough_to_skill`] — legacy conversion that routes through an
//!    intermediate `Workflow` graph for callers that already hold a synthesized
//!    draft. Retained for existing tests; the Tauri command surface no longer
//!    calls it.
...
// ── Legacy path: Workflow graph → Skill ────────────────────────────────

/// Legacy conversion that routes through an intermediate `Workflow` graph.
/// Retained for existing tests. The Tauri command surface no longer calls this.
...
mod tests {
    ...
    // ── Legacy walkthrough_to_skill tests ───────────────────────────────
